...
LOGFILE=/home/amin/logs/cnb.log

echo -e "check if anything to process - exit otherwise\n"
if [ -e "$CNB_LOCAL_DATA_FILE" ];
then 
	if ! [[ "`wc -l $CNB_LOCAL_DATA_FILE | awk '{ print $1}'`" -gt "0" ]]; then echo -e "`date` - Fail: no data to process" >> $LOGFILE; exit 1; fi
...
if ! [[ "$?" -eq "0" ]]; then echo "`date` - Fail: copy to master" >> $LOGFILE; exit 3; fi
...
if ! [[ "$?" -eq "0" ]]; then echo "`date` - Fail: copy to hadoop" >> $LOGFILE; exit 4; fi
...
if ! [[ "$?" -eq "0" ]]; then echo "`date` - Fail: copy to internal table" >> $LOGFILE; exit 5; else echo "Success @ `date`" >> $LOGFILE; fi
